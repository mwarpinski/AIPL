...
### P3 — Define integer semantics once; add VM-vs-wasm differential testing [NOT DONE — semantics half landed, testing half missing]
...
| `tests/test_differential.rs` | **Not done** | File does not exist. `tests/` contains only `test_all.rs`, `test_diagnostics.rs`, `test_opcode_conformance.rs`, `test_v2.rs`. |
| `wasmtime` dev-dependency | **Not done** | `Cargo.toml` `[dev-dependencies]` has only `wasmparser`. No VM-vs-wasm execution comparison exists anywhere in the repo. |
| Explicit cases `(+ 2147483647 1)`, `(shr -8 1)`, `(* 65536 65536)`, `(/ -7 2)`, `(% -7 2)`, inclusive loop bound, while with `set!` | **Not done** | No test asserts any of these values in either backend. |
| "wasm semantics are the spec" at top of AIPL_SPEC.md | **Not done** | `grep -i "wasm semantics" AIPL_SPEC.md` returns nothing. |

**Known divergence the differential test would catch (fix in the VM, per the rule above):** the VM `loop` adds an overflow guard (`if st_val > 0 && curr < s_val { break; }` at [vm.rs:214](src/vm.rs#L214)) that the wasm lowering does not have ([wasm.rs:452-464](src/compiler/wasm.rs#L452-L464) uses `I32GtS` + `BrIf` with no wrap check). A `(loop i 0 2147483647 1 ...)` terminates in the VM and spins forever in wasm. Since wasm is the reference, the VM guard must go (or the spec must define loop-bound overflow as a checked error in both backends).

**Remaining prompt (run this to finish P3):**

```
Repo: AIPL. P3's integer semantics are already implemented (wrapping ops, ShrU/DivU/RemU, i64 rejected in parse_type) — do not redo them. Finish the testing half: (1) Add `wasmtime` as a dev-dependency in Cargo.toml. (2) Create tests/test_differential.rs with a helper `assert_vm_wasm_agree(src: &str, fn_name: &str)` that parses + type-checks `src`, runs `fn_name` (zero-arg, returns i32) in aipl_core::vm::VM, compiles the module with aipl_core::compiler::wasm::WasmCompiler, instantiates the bytes in wasmtime, calls the exported function, and asserts both i32 results are equal (or that both error). (3) Add one #[test] per explicit case: (+ 2147483647 1) → -2147483648; (shr -8 1) → -4; (shru -8 1) → 2147483644; (* 65536 65536) → 0; (/ -7 2) → -3; (% -7 2) → -1; (divu -1 2) → 2147483647; (remu -1 2) → 1; a `loop` from 0 to 5 step 1 summing the counter → 15 (proves inclusive end bound); a `while` that mutates a `let` via `set!` in its body; `shl` by 33 (mask to 1). (4) Add a #[test] that iterates every examples/*.aipl file, parses it, and for every zero-arg fn returning i32 runs the differential assertion — skip a file with a logged reason only if it uses ops the wasm backend rejects (sys.print, fs.*, thread.*, atomics, floats in memory). (5) Add a #[test] for aipl_src/codegen.aipl's `test_signatures_and_locals` if it is zero-arg i32 and wasm-compilable; otherwise document why it is skipped. (6) Fix every divergence in src/vm.rs, never in src/compiler/wasm.rs. Start with the known one: remove the overflow-break guard at vm.rs Expr::Loop (`if st_val > 0 && curr < s_val { break; }`) or make both backends trap on counter overflow — pick one and apply it to both. (7) Update AIPL_SPEC.md: add a line directly under the title reading "Integer semantics: wasm semantics are the spec. `i32` and `i64` are wrapping two's-complement; the VM must match wasmtime bit-for-bit, and any divergence is a VM bug." (i64 is now a real type — see section 8.1 — so the differential cases must also cover `(+ 9223372036854775807i64 1i64)`, `(i32.wrap 4294967301i64)`, `(i64.extend_u -1)`, `(shl 1i64 65i64)`, and an `if` with i64 branches, run through wasmtime with an i64 result.) (8) Update PROGRESS.md and LANGUAGE_GAPS.md. Run `cargo test`; every differential case must pass. Do not mark this item done until tests/test_differential.rs exists and is green.
```
