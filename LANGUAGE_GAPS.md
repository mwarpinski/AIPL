...
### Compiled I/O via WASI (P6, 2026-09-18)

The wasm backend now lowers `sys.print`, `sys.exit`, and all five `fs.*` ops to `wasi_snapshot_preview1` imports (`fd_write`, `fd_read`, `path_open`, `fd_close`, `proc_exit`, `path_unlink_file`), importing only what a module uses, and interns string literals into a data segment at 512-1023 as `[len][bytes]` with `(str.len s)` reading the length. `aipl_src/file_io.aipl` compiles and its self-test passes under wasmtime with a preopened directory, matching the VM (`tests/test_wasi.rs`). What is still missing on the string/I/O side:

- **String concatenation** `(+ str str)` is VM-only; the wasm backend rejects it. Doing it in wasm needs a small runtime (allocate `len1+len2+4`, copy) that does not exist yet.
- ~~No `str` -> `(ptr, len)` conversion.~~ Closed: `(str.ptr s)` + `(str.len s)` give the pair `fs.*` take, in both backends (`examples/word_count.aipl` uses them for its path and messages).
- **`sys.print` of non-`str` values** works in the VM (Debug formatting) but is rejected by the wasm backend; there is no built-in integer-to-string routine. It can be done in AIPL today by formatting digits into a `mem.alloc` buffer and writing them to fd 1 with `fs.write` (see `print_uint` in `examples/word_count.aipl`); a standard-library module for this is the natural next step.
- **`sys.exit` in the VM** does not terminate the host; it returns the error `sys.exit(N) requested`. The CLI does not yet map that to a process exit code.
- **`sys.time`** is unimplemented in both backends. `thread.*` and `atomic.*` remain VM-only (wasm needs shared memory + wasi-threads).
- **`codegen.aipl`** (self-hosted) knows nothing about strings, imports, or WASI yet, and does not implement the mixed-void `if` top-up the Rust backend now performs for `(if c (set! x v) 0)`. This is deliberately left for **P9** in `AIPL_Structural_Audit.md`, whose prompt was rewritten to require whole-module byte equality with the Rust backend (imports, string data segment, WASI lowerings, store guard, locals layout, memory limits); it is not a P6 leftover. On the plus side, since P6 the whole `codegen.aipl` + `compiler.aipl` module compiles to wasm and its `test_signatures_and_locals` runs under wasmtime with the same result as the VM (`tests/test_differential.rs`).

### One memory layout, one allocator (P5, 2026-09-18)

The three independent heap cursors (VM `heap_ptr` field, wasm mutable global, `memory.aipl`'s private word at address 0 starting at 8192) are now one: the i32 at linear-memory address 0, initialised to 1024 by `VM::new` and by a wasm data segment, bumped by `mem.alloc` in both backends and by `memory.aipl`'s thin wrapper. Bytes 0-1023 are a documented runtime block; `codegen.aipl` keeps its table pointers in cells 16-28 and allocates the tables with `mem.alloc`, so no `.aipl` module in `aipl_src/` hardcodes a table or buffer address anymore. `mem.grow` exists in both backends. Both backends now start at 16 pages and cap at 100. The layout is enforced where it can be: the checker rejects literal-address memory/atomic ops that write bytes 0-3, touch bytes 64-1023, or use a misaligned runtime cell; the VM refuses to spin on a lock word that is not 0/1 and to unlock a word that is not 1, so a computed address that lands on the cursor is an error, not a hang. Computed addresses are covered too: every `mem.store*` and `atomic.*` op checks its address at runtime in **both** backends (VM error, wasm `unreachable` trap, emitted by the Rust backend before each store), so a write into 0-3 or 64-1023 is never silent no matter how the address was built. The self-hosted `codegen.aipl` emits the same guard: `emit_store_guard` produces the identical instruction bytes before every `mem.store*` it compiles, every function declares the extra scratch local, and `tests/test_selfhost.rs` proves both that the self-hosted output traps on reserved addresses in wasmtime and that its function body is byte-for-byte equal to the Rust backend's for the same one-store program. Remaining gaps: `mem.free` is still a no-op (bump allocator never reclaims), reads from the reserved block are deliberately unchecked, and `codegen.aipl` still does not compile `mem.alloc`/`mem.grow` at all (so its output has no data segment seeding the cursor; the 16-page memory minimum now matches).

### VM-vs-wasm agreement is now tested (2026-09-18)

`tests/test_differential.rs` runs every integer edge case, control-flow shape, the i64/f64 cases, and every all-`i32`-parameter function in `examples/*.aipl` in both the VM and wasmtime (dev-dependency) and asserts identical results or identical failure. The only tolerated asymmetry is a VM contract failure, because the wasm backend emits no contracts. Two things still cannot be compared: `hello_browser.aipl` (uses removed `dom.*` ops; pinned as stale) and `aipl_src/codegen.aipl` (its `test_compile_*` functions use `fs.*`, so the module does not compile to wasm until P6 lands). The one divergence found, the VM `loop` overflow guard, was removed from the VM.
